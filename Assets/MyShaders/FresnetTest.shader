Shader "Unlit/FresnelTest" // shader name  - shader1 under shader category - Unlit
{
    ...
    Properties
    {
        ...
        _centerStrength("CenterStrength", Float) = 0.5 //with a specified range.
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float3 color = dot(i.viewDirection, i.worldNormal);
                float4 finalColor = float4(color, 1);
                finalColor = 1-saturate(finalColor);
                finalColor = pow(finalColor, _centerStrength);
                return finalColor;
